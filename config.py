...
TELEGRAM_ACTIVITY_FILE = DASHBOARD_DIR / "telegram_activity.json"
TELEGRAM_THREADS_FILE = DASHBOARD_DIR / "telegram_threads.json"
PM_THREADS_FILE = DASHBOARD_DIR / "pm_threads.json"
...
TELEGRAM_PM_ONLY_MODE = (os.getenv("TELEGRAM_PM_ONLY_MODE", "true").strip().lower() in {"1", "true", "yes", "on"})
PM_DISPATCH_ENABLED = (os.getenv("PM_DISPATCH_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"})
PM_CODEX_ENABLED = (os.getenv("PM_CODEX_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"})
PM_CODEX_MODE = os.getenv("PM_CODEX_MODE", "cloud_sdk").strip() or "cloud_sdk"
CODEX_BRIDGE_URL = os.getenv("CODEX_BRIDGE_URL", "").strip()
CODEX_REPO_SLUG = os.getenv("CODEX_REPO_SLUG", "").strip()
FIGMA_ACCESS_TOKEN = os.getenv("FIGMA_ACCESS_TOKEN", "").strip()
